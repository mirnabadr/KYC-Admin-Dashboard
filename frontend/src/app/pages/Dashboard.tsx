...
import { mockTransactions, mockAuditLogs, mockChartData } from '../data/mockData';
import { fetchUSDtoUSDCRate, CybridRate } from '../data/cybridApi';
import { ArrowUpRight, ArrowDownRight, CheckCircle2, XCircle, Clock, DollarSign, TrendingUp, Activity } from 'lucide-react';
...
export function Dashboard() {
  ...
  const rejectedCount = mockTransactions.filter(t => t.status === 'Rejected').length;
  ...
  const regionData = [
    { name: 'US', value: mockTransactions.filter(t => t.region === 'US').length, color: '#3b82f6' },
    { name: 'EU', value: mockTransactions.filter(t => t.region === 'EU').length, color: '#8b5cf6' },
    { name: 'APAC', value: mockTransactions.filter(t => t.region === 'APAC').length, color: '#10b981' },
    { name: 'LATAM', value: mockTransactions.filter(t => t.region === 'LATAM').length, color: '#f59e0b' },
  ];
        ...
        <div className="group">
          <GlassCard hover className="p-6">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">Cybrid API</p>
            </div>
          </GlassCard>
        </div>
      </div>
